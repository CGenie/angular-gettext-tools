...
var mkAttrRegex = function (startDelim, endDelim, parse_filter) {
    var start = startDelim.replace(escapeRegex, '\\$&');
    var end = endDelim.replace(escapeRegex, '\\$&');
    var translate_filter = '(:\\S.*)*';
    if(!parse_filter) {
        translate_filter = '';
    }
    ...
    return new RegExp(start + '\\s*(\'|"|&quot;)(.*?)\\1\\s*\\|\\s*translate' + translate_filter + '\\s*(' + end + '|\\|)', 'g');
};
...
var Extractor = (function () {
    function Extractor(options) {
        ...
        this.strings = {};
        this.attrRegex = mkAttrRegex(this.options.startDelim, this.options.endDelim, this.options.parse_filter);
    }
